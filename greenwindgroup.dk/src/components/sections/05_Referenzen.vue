      ...
      .container.content
        .columns
          ...
          .text.column.is-6.is-offset-3.has-text-centered
            h2 {{ c.t1 }}
            p {{ c.t2 }}
            //- h2 EFFEKTIVITET OG ENGAGEMENT PÅ STEDET
            //- h2 Effektivitet og engagement på stedet
            //- p Som en del af Green Wind Group udvikler Green Wind Energy vindenergi projekter til regionen. I Tyskland og Danmark. Vi er den rigtige partner, når det handler om fortætning af eksisterende vindmølleparker, om re-powering og også om nye udviklinger med samarbejdspartnere.
            //- p Se vores projekter her, herunder en af de største tyske re-powering parker i 2017 i Brandenburg, og flere andre.
      ...
      .container.content
        ...
        section.section.text(v-for="sections in c.sections")
          ...
          .columns(v-for="sub in sections.subsections")
            .column.is-5(v-if="sub.video || sub.img || sub.img2")
            ...
            .column.is-6.is-offset-1
              .legend-wrapper(v-if="section === 'operations'")
                img(v-if="lang === 'dk'", :src='sub.legend').legend
